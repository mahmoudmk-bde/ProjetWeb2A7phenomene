...
if (isset($_POST['face_login']) && isset($_POST['descriptor'])) {
    ...
    $allUsers = $utilisateurc->getAllFaces(); // Renamed method
    $matchFound = false;
    $matchedUserId = null;

    foreach ($allUsers as $user) {
        $storedDescriptor = json_decode($user['face']); // Column name is 'face'
    ...
    if ($matchFound) {
        ...
        if ($user) {
            ...
            $redirect = ($user['typee'] === 'admin') 
                ? 'http://localhost/tache%20utilisateur/view/backoffice/admin.php' 
                : 'index1.php';
  ...
  <div class="login-container">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // URL des modèles (fallback sur le repo officiel si pas local)
            ...
            video.addEventListener('play', () => {
                ...
                const displaySize = { width: 480, height: 360 }; // Fixed size matching video attributes
                ...
                const interval = setInterval(async () => {
                    ...
                    if (detections.length > 0) {
                        ...
                        if (detections[0].descriptor) {
                            ...
                            // Optionnel: Pause pour éviter spam
                            // clearInterval(interval); 
                            ...
                            .then(data => {
                                if (data.success) {
                                    ...
                                    clearInterval(interval); // Stop scanning on success
                                    stopVideo();
                                    setTimeout(() => window.location.href = data.redirect, 1000);
                                } else {
                                    // Log failure but keep trying or wait a bit
