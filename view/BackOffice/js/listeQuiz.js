...
function updateQuizStats() {
    const totalQuizzes = document.querySelectorAll('.data-table tr').length - 1;
    const statsContainer = document.querySelector('.quiz-stats');
    
    if (statsContainer) {
        const uniqueArticles = new Set();
        document.querySelectorAll('.article-id-cell').forEach(cell => {
            uniqueArticles.add(cell.textContent.trim());
        });
        ...
        statsContainer.innerHTML = `
            ...
            <div class="stat-card">
                <div class="stat-number">${uniqueArticles.size}</div>
