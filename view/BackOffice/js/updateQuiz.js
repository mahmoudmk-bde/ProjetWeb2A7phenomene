document.addEventListener('DOMContentLoaded', function() {
    ...
    // === VALIDATION DU FORMULAIRE DE MODIFICATION QUIZ ===
    ...
    function validateArticle() {
        const input = document.getElementById('id_article');
        if (!input) return false;
        
        const value = input.value.trim();
        if (!value) {
            showError(input, 'L\'ID article est obligatoire');
            return false;
        }
        
        if (isNaN(value) || value <= 0) {
            showError(input, 'L\'ID article doit être un nombre positif');
            return false;
        }
