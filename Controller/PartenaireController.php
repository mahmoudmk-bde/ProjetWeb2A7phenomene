...
class PartenaireController {
    ...
    public function index() {
        ...
        if ($type && in_array($type, ['sponsor','testeur','vendeur'])) {
            $partenaires = array_values(array_filter($partenaires, function($p) use ($type){
                return strtolower(trim($p['type'] ?? '')) === $type;
            }));
        }
    ...
    public function show() {
        if (isset($_GET['id'])) {
            ...
            if ($this->partenaire->getById() && $this->partenaire->statut === 'actif') {
                ...
                if (!empty($jeux)) {
                    $statsTotalLikes = array_sum(array_map(function($j){return (int)($j['likes_count'] ?? 0);}, $jeux));
                    $statsTotalViews = array_sum(array_map(function($j){return (int)($j['views_count'] ?? 0);}, $jeux));
                    $statsAvgPrice = array_sum(array_map(function($j){return (float)($j['prix'] ?? 0);}, $jeux)) / count($jeux);
                    $ratings = array_values(array_filter(array_map(function($j){return isset($j['rating_avg']) ? (float)$j['rating_avg'] : null;}, $jeux), function($v){return $v !== null;}));
                    if (count($ratings) > 0) {
                ...
                try {
                    ...
                    if ($r && $partnerRatingCount === 0) { $partnerRatingAvg = (float)$r['avg_score']; $partnerRatingCount = (int)$r['cnt']; }
